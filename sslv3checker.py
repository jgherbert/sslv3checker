...
for host in hosts:
    ...
    if (pid == 0):
        ...
        if (re.search('^\s*https://',host)):
            ...
            #It isn't http, it isn't https, yet we still have // in there? Ditch
            print "Invalid URI".rjust(40) + ":  !! " + host
            os._exit(0)
         
        # Now check for a /path on the end and split it off as it's not necessary
        # No mercy! 
        host = re.sub('/.*$','',host)
